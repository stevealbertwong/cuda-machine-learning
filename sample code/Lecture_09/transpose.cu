...
/*
1 col == same bank / cache line 
this is the problem of global memory
*/

// warp size in GPU is fixed at 32 
...
r() == coalesced (threads of same warp + adjacent data == no GPU cache miss)
w() == uncoalesced (many GPU cache misses)

assume 8 threads each block == move in 8x1 vector
...
void simpleTranspose(int* array_in, int* array_out, size_t n_rows, size_t n_cols) {
  ...
  // 1. one array of threads convert into matrix col, row 
  // each thread 1 unique col, row 
  size_t col = tid % n_cols;
  size_t row = tid / n_cols; // same warp since tid close to each other

  // 2. strategy: just reposition to the new pos in transposed matrix
  if(col < n_cols && row < n_rows) {
    array_out[col * n_rows + row] // non adjacent data w()
      = array_in[row * n_cols + col]; // adjacent data r()
  }
}
...
slightly better than simpleTranspose()
2D == move in 4x4 square
*/
__global__
void simpleTranspose2D(int* array_in, int* array_out, size_t n_rows, size_t n_cols) {
  ...
  // 1. strategy: just reposition to the new pos in transposed matrix
  if(col < n_cols && row < n_rows) {
...
void fastTranspose(int* array_in, int* array_out, size_t n_rows, size_t n_cols) {
  ...
  __shared__ int block[warp_size][warp_size+1]; // variable allocated in shared memory
